...
// M71: bulk dungeon room interaction logic extracted to DungeonRoomInteractionSystem
import { tickDungeonRoomInteractions } from './DungeonRoomInteractionSystem'
...
export function GameLoop({ controllerRef, simManagerRef, entityId, gameActive }: GameLoopProps) {
  ...
  // M71: puzzleResetCheckRef removed — dungeon room logic moved to DungeonRoomInteractionSystem
  ...
  useFrame((_, delta) => {
    ...
    // ── M36 Track B: Dungeon room interactions (M71: extracted to DungeonRoomInteractionSystem) ──
    {
      const dungeonResult = tickDungeonRoomInteractions({
        px, py, pz, dt, entityId,
        dungeonRoomCheckTimer: dungeonRoomCheckRef.current,
        gs: { inputBlocked: gs.inputBlocked, gatherPrompt: gs.gatherPrompt, setGatherPrompt: gs.setGatherPrompt },
        controllerRef,
      })
      dungeonRoomCheckRef.current = dungeonResult.dungeonRoomCheckTimer
    }

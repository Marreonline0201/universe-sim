...
## M41 new systems (session 10)

- AlchemyPanel.tsx (Y key, sidebar ALK): 3 ingredient slots + BREW button. Lazy-loaded in SidebarShell, Y hotkey wired. Reads CRAFTING_RECIPES filtered by requiresAlchemyTable. checkAlchemyMutation() from PotionSystem. discoveriesLog for mutation history. Standard recipes use inventory.craft(). Mutation recipes use findItem()+removeItem() per slot.
- MountHUD.tsx: Shown in HUD.tsx wrapped in Suspense. Reads all state from useMountStore with granular per-field subscriptions (reactive). Shows HP bar + stamina bar + GALLOPING badge. Lazy-loaded. Correctly hidden when mountedAnimalId === null.
- mountStore.ts: Zustand store with mount/dismount/setMountHealth/setMountStamina/setGalloping. mountSpeed default 1.4. isGalloping default false.
- Mount R-key: PlayerController.popMount() uses KeyR (not pointer-lock gated), consumed-flag pattern. GameLoop reads keys directly for ShiftLeft/ShiftRight for gallop. isGalloping updated if key state differs.
- setMountSpeedMult: module-level let in PlayerController.ts (line 24-25). Applied in applyMovement() line 451 — WALK_SPEED * speedMult * weatherSpeedMult * getFoodSpeedMult() * mountSpeedMult. Sprint/crouch disabled while isMounted (mountSpeedMult > 1.0).
- FestivalSystem.ts: singleton FestivalSystemClass. tick(dt, season, dayDurationSeconds). Season-change detection via _lastActiveSeason guard. Festival ends after durationDays in-game days. CustomEvents: festival-start / festival-end. 4 festivals: harvest_feast (AUTUMN), winter_solstice (WINTER), spring_bloom (SPRING), summer_hunt (SUMMER).
- festivalStore.ts: Zustand store, sync() pulls from festivalSystem singleton. Called each frame from GameLoop (line 2662).
- FestivalHUD.tsx: Listens to festival-start/festival-end events for banner. Reads activeFestival from useFestivalStore for corner indicator. Auto-dismiss banner 8s. Season colors defined. Wrapped in Suspense in HUD.tsx.

## Known bugs / issues (updated 2026-03-26 session 10 / M41 playtest)

CRITICAL:
- ALCHEMY MUTATION CONSUME LOGIC IS NOT ATOMIC — PARTIAL CONSUME ON FAILURE (M41 Track A): AlchemyPanel.tsx handleBrew() line 99 uses `slots.every(s => { ... return inventory.removeItem(...) })`. Array.every() short-circuits on false, meaning if slot 0 removes successfully but slot 1 fails, slot 0's ingredient is already consumed and lost. The player loses an ingredient with nothing brewed. Fix: collect slot indices first (findItem per mat), validate all exist, then remove all three in sequence.
- ALCHEMY MUTATION DISCOVERY FIRES ON EVERY BREW ATTEMPT, NOT JUST FIRST (M41 Track A): PotionSystem.ts checkAlchemyMutation() calls addDiscovery() unconditionally on every call, even when the combo has already been discovered. addDiscovery() deduplicates in discoveriesLog, but checkAlchemyMutation() is also called during previewLabel rendering (AlchemyPanel line 75) — so discovery is logged every time the user fills all 3 slots, even before pressing BREW. Fix: only call addDiscovery() inside handleBrew, not in the preview path.
- MOUNT STAMINA EXHAUST CHECK READS STALE VALUE (M41 Track B): GameLoop.ts line 445 calls `mountSt.setMountStamina(curStamina - 15 * dt)`, then line 447 checks `if (mountSt.mountStamina <= 0)`. `mountSt.mountStamina` still holds the PRE-update value because Zustand set() is async — the store hasn't re-rendered yet. On a 60fps frame with dt=0.016, drain is 0.24 per frame. The check will never see 0 until the next frame's getState() call, meaning dismount fires one frame late (minor) but more importantly if curStamina starts at 0.1, the check `curStamina > 0` on line 442 passes, drains to -0.14, and the stale read still sees 0.1 — the exhaust branch never fires that frame. Fix: check `(curStamina - 15 * dt) <= 0` inline instead of re-reading from store.
- ALCHEMY-MUTATION CUSTOM EVENT DISPATCHED BUT NEVER LISTENED (M41 Track A): AlchemyPanel.tsx line 106 dispatches `window.dispatchEvent(new CustomEvent('alchemy-mutation', ...))` but no listener exists anywhere in the codebase. The event is dead. The intended effect (giving the player a fire/shock potion item or adding a spell) never happens. The player brews a mutation, consumes the ingredients, sees the notification, but receives nothing.
- CIV PROGRESSION SYSTEM FULLY UNWIRED (M39 Track C): civStore, CivMilestoneSystem, and checkAndFireMilestones are defined but never imported into GameLoop.ts or HUD.tsx. setTotalTier() is never called — civLevel stays 'stone_age' forever. pendingLevelUp is never displayed — no banner component exists anywhere. All milestone bonus getters (getCraftCostMultiplier, getSkillXpMultiplier, etc.) are exported but called from zero locations. The entire Track C feature is dead code.

IMPORTANT:
- MOUNT R-KEY WORKS WITHOUT POINTER LOCK (M41 Track B): PlayerController.onKeyDown() (line 283) has NO pointer lock guard — it adds any key to `this.keys` unconditionally. popMount() therefore fires when the user presses R in a browser context without pointer lock (e.g., while a panel is open, or after clicking the page body). This can trigger spurious mount/dismount when typing R in chat or other UI. Mitigating factor: SidebarShell blocks input when a panel is open, but GameLoop's mount block still runs when panelOpen. Low severity in practice.
- FESTIVAL SYNC CALLED EVERY FRAME BUT festivalSystem.tick() ONLY RUNS ONCE (M41 Track C): GameLoop tick block at line 2658-2663 calls `festivalSystem.tick(dt, currentSeason, 1200)` then `useFestivalStore.getState().sync()`. sync() calls Zustand set() every single frame even when no values changed. This causes unnecessary React re-renders on FestivalHUD every frame while a festival is active (because `activeFestival` object reference stays the same, but `festivalDayCount` increments). Low perf impact but worth throttling or doing a shallow equality check before calling set().
- SKY GRADIENT UPDATE FIRES EVERY FRAME (M39 Track A): DayNightCycle.tsx line 305 checks `if (skyUpdateTimer.current === 0)` immediately after the block that resets it to 0 at line 292. The condition is always true on the same frame the throttle fires — which is correct in intent but relies on the reset happening before the check in the same frame. However: the `horizP` variable used on line 307 is recalculated without the sunAboveHorizon guard, shadowing the one from line 293 scope, but this is actually a separate let declaration so it is correct. This is a latent confusion but not an active bug. LOWER PRIORITY than initially thought.
- GOD RAY FAN ROTATION MATH IS WRONG (M39 Track A): DayNightCycle.tsx lines 354-356 rotate godRayX/godRayZ using a 2D rotation matrix around Y-axis, but then discards godRayY and uses it unrotated. The fan spread applies in the XZ plane of the absolute position, not relative to the sun direction vector. Rays will spread in world-space XZ instead of fanning relative to sun azimuth. Visually subtle but rays will diverge incorrectly when sun is not near azimuth=0.
- MILESTONE SKILL XP BONUS NEVER APPLIED (M39 Track C): SkillSystem.addXp() (line 227) multiplies by baseMultiplier * prestigeXpBonus * partyBonus but never calls getSkillXpMultiplier() from CivMilestoneSystem. Even if civStore were wired, the 'knowledge_spreads' +15% bonus would have no effect.
- DIALOGUE ROLES HEALER/BLACKSMITH/SCHOLAR MISSING FROM ROLE_DIALOGUE (M36/M39): DialoguePanel.tsx lines 666, 698, 775 check role === 'healer', 'blacksmith'/'smith', 'scholar'/'librarian' for service actions. But ROLE_DIALOGUE (line 81) has no entries for these roles. GameLoop NPC_ROLES array (line 1098) also does not include these roles — NPCs are only assigned: villager/guard/elder/trader/artisan/scout. So healer/blacksmith/scholar service NPCs can never be spawned via the standard NPC system, and if they were (e.g., from M38 schedule system), they would fall through normaliseRole() returning null, giving them no dialogue pool. The service action buttons are orphaned.
...
KNOWN PRESENT (previously listed, still not fixed as of M39):
- HUD.tsx line 2180: `label="Stamina"` on the fatigue bar (value={1-fatigue}). Should be "Endurance" to distinguish from the new StaminaBar (M38 Track B) which also appears directly below it on line 2184. With both bars visible, players see two purple-area bars — one labeled "Stamina" and one unlabeled — causing confusion about which is which.
- Q-KEY CONFLICT: Q opens Quests panel (SidebarShell) AND is the attack key in-game. Split by pointerLockElement. Intentional but potentially confusing.
- CHEMISTRYHUD REACT HOOK BUG (session 5): ChemistryHUD.tsx useEffect at line 22 has `events.length` in dependency array — stale closure.

NICE TO HAVE (M41 additions):
- POTION SPEED MULT NOT APPLIED TO MOUNTED MOVEMENT: getPotionSpeedMult() is defined and exported but called from zero places in PlayerController or GameLoop. The Speed Potion has no effect on mounted movement speed. Also not applied to unmounted walking either — confirmed by grepping for getPotionSpeedMult.
- ELIXIR OF WISDOM (id 156) ALCHEMY RECIPE HAS 4 INPUTS: AlchemyPanel only checks recipes where `inputs.length === filledMats.length` and the slot array is capped at 3. Recipe 156 has 4 inputs (Golden Fish + 3 Alcohol + 2 Mushroom + 1 Vial = 4 distinct entries) — it can never be matched in the 3-slot alchemy panel. Players can see it in KNOWN RECIPES list but can never brew it from AlchemyPanel.
- DISCOVERIES LOG NOT REACTIVE: AlchemyPanel reads `discoveriesLog` (a plain mutable array) directly in JSX. The 300ms forceRefresh timer picks up changes, but this is the only driver — there's no event-driven update when a discovery happens. The banner fires immediately (via notification), but the Discoveries section in the panel itself may lag up to 300ms.
